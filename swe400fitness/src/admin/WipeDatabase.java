...
public class WipeDatabase
{
	...
	{
		WipeDatabase wiper = new WipeDatabase();
		wiper.connect();
		Statement wipe = (Statement) wiper.connection.createStatement();
		wipe.executeUpdate("DELETE FROM pendingfriends;");
		wipe.executeUpdate("DELETE FROM friends;");
		wipe.executeUpdate("DELETE FROM people;");
		wiper.closeConnection();
	}
